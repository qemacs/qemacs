...
This is the TODO list for qemacs
...
* try and recover from invalid memory access
...
# Frames - Windows - Rendering - Displays

## Frames

* Add frames on text displays
* Add frames on x11 displays
...
* vertical scroll bar
* menu / context-menu / toolbars / dialogs / menubar
...
## Rendering: producing window contents
...
* cursor not found bug on **doc/256colors.raw** if `truncate-lines=1`
...
* outline view: show/hide regions based on syntax
* auto pretty view: insert indentation and line breaks based on syntax
...
* support graphics display on capable terminals
...
* resync from end of line without threshold if subsequent lines match
* `visual-diff-mode`: Use color-coding to compare two buffers.
* autoblame source code
* `diff-buffer-with-staged`, `diff-buffer-with-head`, `diff-buffer-with-master`
* builtin git status and commands

# Input - Macros - Completion - Commands
...
* minibuffer completion: bad return on `C-x C-f . SPC qe SPC RET`
...
* pass has_arg and arg_val as separate arguments or combined 64-bit value
...
* remote editing
* `delete-selection-mode` to delete the highlighted region on DEL and typing text
* `whitespace-cleanup`
...
* `c-mode` descendants:
  ...
  * `vala-mode`
  * `cuda-mode`
  * `odin-mode`: c-mode indentation does not work because of lack of semicolons
* `electric-c-mode`
...
* for code blocks without a language tag, try and determine the language from contents
...
## Man command and mode

* control where `man` window is opened with prefix and setting
...
* integrate qscript or MicroQuickJS

## Debugging

* add command line options for tracing
* add script variable for tracing
* track line changes in next-error matches using array of locations
